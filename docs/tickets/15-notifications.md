...
### 通知作成
- [x] リアクションinsert後にDBトリガーで `notifications` テーブルへinsert
- [x] コメントinsert後にDBトリガーで `notifications` テーブルへinsert
...
## 実装ファイル
- `lib/notifications.ts` — `fetchNotifications` / `countUnreadNotifications`
- `app/_actions/social.ts` — リアクション/コメントをinsert。通知宛先は指定しない
- `supabase/migrations/0016_security_hardening.sql` — 通知作成トリガーと直接insert権限の廃止
